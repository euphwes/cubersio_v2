...
# These 3 paths describe the shape of a piece on a face of a megaminx.
BASE_EDGE_PATH = parse_path("M 438 95 L 475 122 L 427 271 L 311 187 Z")
BASE_CORNER_PATH = parse_path("M 310 0 L 437 94 L 310 186 L 183 94 Z")
BASE_CENTER_PATH = parse_path("M 310 187 L 427 272 L 382 409 L 238 409 L 193 272 Z")
...
def get_paths_for_template_face():
    ...
    paths = [
        Path(
            *[
                (segment.scaled(0.9).translated((31 + 31j)))
                for segment in list(BASE_CENTER_PATH)
            ]
        )
    ]
    ...
    for angle in [0, 72, 144, 216, 288]:
        ...
        # Apply the transformations to each segment and generate a new path for the edge
        paths.append(
            Path(
                *[
                    (
                    ...
                    for segment in list(BASE_EDGE_PATH)
                ]
            )
        )
